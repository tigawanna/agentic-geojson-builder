...
function parsePermissionLabel(perms: Record<string, string[]> | null | undefined): string {
  ...
  const mapProjectPerms = perms.mapProject ?? [];
  if (mapProjectPerms.includes("delete") || mapProjectPerms.includes("update"))
    return "Read & Write";
  if (mapProjectPerms.includes("view") || mapProjectPerms.includes("list")) return "Read only";
  return "Custom";
}
