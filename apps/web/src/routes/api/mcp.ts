...
import { createGeojsonMcpServer } from "@/features/agentic-tools/geojson-mcp.server";
...
async function mcpHandler(request: Request): Promise<Response> {
  const authResult = await authenticateApiKeyRequest(request, { mapProject: ["view"] });
  ...
  const server = createGeojsonMcpServer(authResult.userId);
